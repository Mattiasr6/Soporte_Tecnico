...
export default function ChatWidget() {
  ...
  const dragRef = useRef<{ offX: number; offY: number }>({ offX: 0, offY: 0 });
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      setPos({
        x: Math.max(0, Math.min(e.clientX - dragRef.current.offX, window.innerWidth - PANEL_W)),
        y: Math.max(0, Math.min(e.clientY - dragRef.current.offY, window.innerHeight - PANEL_H)),
      });
    };
    ...
    <>
      ...
      {open && (
        <div ref={panelRef}
          ...
          style={isDragged ? { left: pos.x, top: pos.y } : { right: 16, bottom: 80 }}
        >
          <div
            onMouseDown={onMouseDown}
            className="flex cursor-grab items-center justify-between border-b border-slate-700/50 px-4 py-3 select-none"
          >
          ...
          <form onSubmit={enviar} className="flex items-center gap-2 border-t border-slate-700/50 p-3">
            ...
            <button type="submit" disabled={!texto.trim()}
              className="rounded-lg bg-amber-500 px-3 py-2 text-xs font-bold text-slate-900 transition hover:bg-amber-400 disabled:opacity-40">→</button>
          </form>
        </div>
      )}
    </>
